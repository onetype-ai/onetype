onetype.AddonReady('commands', (commands) =>
{
    commands.Item({
        ...
        {
            if(platform.collaborators.ItemGet(properties.id))
            {
                return resolve(null, 'Collaborator ' + properties.id + ' already joined.', 400);
            }

            const palette = ['blue', 'green', 'orange', 'red', 'brand'];
            const color = properties.color || palette[Object.keys(platform.collaborators.Items()).length % palette.length];

            platform.collaborators.Item({
                id: properties.id,

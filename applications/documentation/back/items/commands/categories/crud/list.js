...
commands.Item({
	id: 'documentation:categories:list',
	exposed: true,
	in: {
		filters: {
			type: 'array',
	...
	out: {
		items: {
			type: 'array',
			value: [],
			each: { type: 'object', config: 'documentation.category' }
		},
		total: ['number', 0, true]
	},
	callback: async function(properties, resolve)
	{
		const query = documentation.categories.Find().filter('deleted_at', null, 'NULL');
		...
		const [items, total] = await Promise.all([
			query.many(),
			documentation.categories.Find().filter('deleted_at', null, 'NULL').count()
		]);

...
platform.packages.Fn('sync', function()
{
    ...
    {
        return platform.config.one('packages').Get('value').find((entry) => entry.slug === slug);
    };
    ...
    for(const directory of this.methods.directories())
    {
        ...
        const manifest = onetype.DataDefine(JSON.parse(readFileSync(file, 'utf8')), 'platform.packages.manifest', true);

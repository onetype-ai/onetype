commands.Item({
	...
	exposed: true,
	description: 'Change the key combination of a shortcut and persist it across reloads. Omit key to restore the default combination and drop the stored override. Emits modules.shortcuts.rebind. Does nothing when the shortcut already uses the requested key.',
	...
	callback: function(properties, resolve)
	{
		...
		const item = modules.shortcuts.ItemGet(properties.id);
		...
		else
		{
			...
			if(!modules.shortcuts.Fn('valid', key))
			{
				return fail('Key ' + key + ' is not a valid combination.', 400);
			}
		}

		if(!modules.shortcuts.Fn('rebind', properties.id, key))
		{

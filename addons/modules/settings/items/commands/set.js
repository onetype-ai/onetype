commands.Item({
	...
	exposed: true,
	description: 'Set a setting value. Persists it through the setting storage, syncs the settings state and emits modules.settings.change. The value must match the setting control, string for input, number for number, boolean for toggle, one of the options for select, array for list.',
	...
	{
		const item = modules.settings.ItemGet(properties.id);
		...
		if(scope && !properties.instance && !modules.settings.Fn('scope.active', scope))
		{
		...
		const suffix = scope ? ' for ' + scope + ' ' + (properties.instance || modules.settings.Fn('scope.active', scope)) : '';

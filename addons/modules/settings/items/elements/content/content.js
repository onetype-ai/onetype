elements.ItemAdd({
	...
	render: function()
	{
		...
		const build = () =>
		{
			...
			if(this.mode === 'search')
			{
				this.results = modules.settings.Fn('search', this.query);
			}
			else if(this.mode === 'scope')
			{
				const selected = this.instance || this.chosen || null;

				this.section = modules.settings.Fn('scoped', { [this.scope]: selected }, '').find((scope) => scope.id === this.scope) || { id: this.scope, label: this.scope, icon: 'category', instances: [], selected: null, items: [] };
			}
			else
			{
				const groups = modules.settings.Fn('groups', '');
		...
		this.On('@addon.item.added', (item) => item.addon.GetName() === 'modules.settings' && build());
		this.On('@addon.item.removed', (item) => item.addon.GetName() === 'modules.settings' && build());
		this.On('modules.settings.change', build);
		this.On('modules.settings.scope', build);
		...
		this.save = (item, value) =>
		{
			...
			$ot.command('modules:settings:set', payload);
		};

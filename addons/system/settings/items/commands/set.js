commands.Item({
	id: 'settings:set',
	exposed: true,
	description: 'Set a setting value. Persists it through the setting storage, syncs the settings state and emits settings.change. The value must match the setting control, string for input, number for number, boolean for toggle, one of the options for select, array for list.',
	...
	{
		const item = settings.ItemGet(properties.id);
		...
		if(scope && !properties.instance && !settings.Fn('scope.active', scope))
		{
			return resolve(null, 'Setting ' + properties.id + ' is scoped to ' + scope + ', pass an instance.', 400);
		}

		$ot.settings.set(properties.id, properties.value, properties.instance || null);

		const suffix = scope ? ' for ' + scope + ' ' + (properties.instance || settings.Fn('scope.active', scope)) : '';

commands.Item({
	id: 'canvas:move',
	exposed: true,
	description: 'Move a canvas item to a new position. Persists the layout and emits canvas.move. Positions apply in the free layout, the stack layout computes its own.',
	...
	callback: function(properties, resolve)
	{
		...
		const blocking = canvas.Fn('collides', { id: properties.id, x: properties.x, y: properties.y, width: item.Get('width'), height: item.Get('height') });
		...
		onetype.Emit('canvas.move', { ids: [properties.id] });

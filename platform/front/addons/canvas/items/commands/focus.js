commands.Item({
	id: 'canvas:focus',
	exposed: true,
	description: 'Focus one canvas item. The camera flies in until the item fills the viewport and everything else fades away. Focus ends through canvas:blur, the Escape key, or a click outside the item.',
	...
	callback: function(properties, resolve)
	{
		...
		settings.Fn('set', 'canvas.focus', { id: properties.id, back: focused ? focused.back : { ...camera } });
		settings.Fn('set', 'canvas.camera', target);

		onetype.Emit('canvas.focus', { id: properties.id });

elements.ItemAdd({
	...
	render: function()
	{
		...
		{
			settings.Fn('set', 'canvas.camera', { ...camera });
		}, 300);
		...
		const refresh = () =>
		{
			let items = canvas.Fn('list');

			if(this.layout === 'stack')
			{
				items = canvas.Fn('stack', items);
			}

			const state = settings.Fn('get', 'canvas.focus', null);

			if(state && !items.find((item) => item.id === state.id))
			{
				queueMicrotask(() => $ot.command('canvas:blur'));
			}

			this.focus = state ? state.id : null;
			this.items = items;
			this.selected = (this.selected || []).filter((id) => items.find((item) => item.id === id));
			this.regions = this.layout === 'free' ? canvas.Fn('regions', items) : [];
			this.edges = canvas.Fn('edges', items);
			this.map = minimap(items);
			this.frame = bounds(items);

			canvas.StoreSet('placed', items);
		};

		this.Compute(refresh);

		this.On('@addon.item.added', (item) => item.addon.GetName() === 'canvas' && refresh());
		this.On('@addon.item.modified', (item) => item.addon.GetName() === 'canvas' && refresh());
		this.On('@addon.item.removed', (item) => item.addon.GetName() === 'canvas' && refresh());

		this.On('apps.switch', refresh);
		this.On('modes.switch', refresh);
		this.On('canvas.move', refresh);
		this.On('canvas.group', refresh);
		this.On('canvas.focus', refresh);
		this.On('canvas.blur', refresh);
		this.On('canvas.link', refresh);
		this.On('canvas.unlink', refresh);

		/* Tidy glides the cards into place, the class arms a transform transition
		   for the re-render that follows on canvas.move. */
		...
		{
			if(change.id === 'canvas.camera')
			{
				Object.assign(camera, settings.Fn('get', 'canvas.camera', { x: 0, y: 0, z: 1 }));
		...
		const measure = () =>
		{
			...
			canvas.StoreSet('viewport', { width: size.width, height: size.height });
		};
		...
		const type = (event) =>
		{
			...
			else if(event.key === 'Enter')
			{
				...
				hit && $ot.command('canvas:jump', { id: hit.id });
		...
		const keys = (event) =>
		{
			...
			if(this.focus)
			{
				if(event.key === 'ArrowRight' || event.key === 'ArrowLeft')
				{
					...
					const next = canvas.Fn('near', this.focus, event.key === 'ArrowRight' ? 1 : -1);

					next && $ot.command('canvas:focus', { id: next });
				}
		...
		{
			this.focus && $ot.command('canvas:focus', { id: this.focus });
		}, 300);
		...
		this.render = (entry) =>
		{
			return canvas.ItemGet(entry.id).Fn('render');
		};
		...
		const live = (moved) =>
		{
			canvas.Fn('edges', moved).forEach((edge) =>
			{
			...
			canvas.Fn('regions', moved).forEach((region) =>
			{
		...
		const haul = (event) =>
		{
			...
			const up = () =>
			{
				...
				{
					$ot.command('canvas:shift', { ids, dx: valid.dx, dy: valid.dy });
				}
				else
				{
					refresh();
		...
		this.down = (entry) =>
		{
			return ({ event }) =>
			{
				...
				const move = (moving) =>
				{
					...
					const blocking = canvas.Fn('collides', { id: entry.id, x: candidate.x, y: candidate.y, width: entry.width, height: entry.height });
				...
				const up = () =>
				{
					...
					{
						$ot.command('canvas:move', { id: entry.id, x: valid.x, y: valid.y });
					}
					else
					{
						refresh();
		...
		this.grab = (region) =>
		{
			return ({ event }) =>
			{
				...
				const up = () =>
				{
					...
					if(valid.dx !== 0 || valid.dy !== 0)
					{
						$ot.command('canvas:shift', { group: region.id, dx: valid.dx, dy: valid.dy });
					}
					else
					{
						refresh();
		...
		this.wire = (entry) =>
		{
			return ({ event }) =>
			{
				...
				const up = () =>
				{
					...
					target && $ot.command('canvas:link', { from: entry.id, to: target });
				};
		...
		this.enter = (entry) =>
		{
			return () =>
			{
				!this.focus && $ot.command('canvas:focus', { id: entry.id });
			};
		};
		...
		this.zoom = (direction) =>
		{
			return () =>
			{
				$ot.command('canvas:zoom', { level: Math.round(camera.z * (direction > 0 ? 1.25 : 0.8) * 100) / 100 });
			};
		};

		this.fit = () =>
		{
			$ot.command('canvas:fit');
		};

		this.tidy = () =>
		{
			$ot.command('canvas:tidy');
		};

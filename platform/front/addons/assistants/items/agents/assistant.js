onetype.AddonReady('ai.agents', (agents) =>
{
	agents.Item({
		...
		state: ({ input }) =>
		{
			const application = settings.Fn('get', 'apps.active', 'builder');
			...
			return {
				...
				application,
				mode: modes.Fn('active', application),
				layouts: Object.values(layouts.Items()).filter((item) => item.Fn('visible')).map((item) => item.Get('id')),

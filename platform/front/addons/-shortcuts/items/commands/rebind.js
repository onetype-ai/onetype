commands.Item({
	...
	callback: function(properties, resolve)
	{
		...
		if(key === undefined)
		{
			if(item.Fn('key') === item.Get('key'))
			{
				return resolve({ id: properties.id, key: item.Get('key') }, 'Shortcut ' + properties.id + ' already uses its default key.');
			}
		...
		shortcuts.Fn('save', properties.id, { key: key === item.Get('key') ? undefined : key });

		onetype.Emit('shortcuts.rebind', { id: properties.id, key });
